...
print(f"标题格式: {tmpl['title_fmt']}")
print(f"平台风格: {platform_label}")
print("")
print("请根据上述对话内容，按以下结构写一篇完整的博文(Markdown 格式)：")
print("")
print(tmpl["structure"])
print("")
if platform_type == "all":
    print("⚠️  --platform all：请生成 4 个文件，分别对应以下平台风格：")
    for p in ["wechat", "juejin", "csdn", "zhihu"]:
        print(f"   - {p}: {platform_styles[p]}")
    print(f"文件名: {session_date}-{template_type}-<平台>-<简短英文标题>.md")
else:
    print(f"文件名: {session_date}-{template_type}-{platform_type}-<简短英文标题>.md")
print(f"保存路径: {articles_dir}/")
print("")
print("要求:")
print("  - 语言: 中文")
print("  - 语气: 按平台风格调整（微信口语 / 掘金硬核 / CSDN 实用 / 知乎辩证 / 通用平衡）")
print("  - 不要出现【AI生成】类标签")
print("  - 该贴代码就贴代码，该贴配置就贴配置（微信平台可减少代码密度）")
print("  - 标题必须符合指定平台的标题调性，从对话内容中提炼，不要用占位符")
print("  - 敏感信息保护: 禁止出现 API Key/Token/真实路径/邮箱/用户名，保持脱敏状态")
print("")
print("  " + "-"*50)
